  ...
  <div class="flex min-h-screen flex-col">
    ...
    <main class="flex flex-1 items-center justify-center px-4 pb-16">
      <div class="w-full max-w-2xl">
        ...
        <!-- Error -->
        <div v-if="error" class="mb-6 rounded-lg bg-destructive/10 p-3 text-center text-sm text-destructive">
          {{ error }}
        </div>

        <!-- Form section with ambient glow -->
        <div class="relative">
          <!-- Ambient background glow -->
          <div class="pointer-events-none absolute -inset-16 -z-10">
            <div class="absolute inset-0 m-auto h-[400px] w-[500px] rounded-full bg-primary/10 blur-[120px]" />
          </div>

          <!-- Username (shared) -->
          <div
            class="mx-auto mb-5 max-w-sm transition-all delay-100 duration-700 ease-out"
            :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0'"
          >
            <label for="username" class="mb-2 block text-center text-xs font-medium tracking-wide text-foreground-subtle uppercase">
          ...
          >
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <!-- Join Room Card -->
              <div class="glass rounded-xl border-white/10 p-6 space-y-4 transition-colors duration-200 hover:border-primary/50 hover:bg-white/5">
                <div class="flex items-center gap-2">
        ...
        <!-- Demo -->
        <div
          class="mt-10 text-center transition-all delay-300 duration-700 ease-out"
          :class="mounted ? 'translate-y-0 opacity-100' : 'translate-y-6 opacity-0'"
        >
          <button
            class="text-sm font-medium text-slate-400 transition-all hover:text-white hover:underline"
            @click="tryDemo"
          >
            Want to test it out alone? Try the offline demo.
          </button>
        </div>
      </div>
    </main>
