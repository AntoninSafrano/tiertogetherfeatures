...
import { ArrowLeft, Download, Calendar, Layers, Copy, Settings2 } from 'lucide-vue-next'
  ...
  <div class="min-h-screen bg-zinc-950">
    ...
    <main class="mx-auto max-w-4xl px-4 py-8">
      ...
      <div v-if="isLoading" class="space-y-4">
        <div class="h-8 w-2/3 rounded bg-zinc-800 animate-pulse" />
        <div class="h-4 w-1/3 rounded bg-zinc-800 animate-pulse" />
        <div class="space-y-1 mt-6">
          <div v-for="i in 5" :key="i" class="h-[100px] rounded bg-zinc-800/50 animate-pulse" />
        </div>
      </div>

      <!-- Error -->
      <div v-else-if="error" class="text-center py-16">
        <p class="text-zinc-500 text-lg">{{ error }}</p>
        <button
          class="mt-4 rounded-lg bg-white/5 border border-white/10 px-4 py-2 text-sm text-zinc-300 hover:bg-white/10 transition-colors"
          @click="router.push({ name: 'explore' })"
        >
          Back to Explore
        </button>
      ...
      <template v-else-if="tierlist">
        ...
        <!-- Tier Rows -->
        <div class="rounded-xl border border-white/10 bg-zinc-900 overflow-visible shadow-2xl">
          <div
          ...
          >
            ...
            <!-- Gear button -->
            <button
              class="absolute right-2 top-1/2 -translate-y-1/2 rounded-md bg-zinc-800/80 p-1.5 text-zinc-400 opacity-0 transition-all hover:bg-zinc-700 hover:text-white group-hover/preview:opacity-100"
            ...
            <div
              ...
              class="absolute right-10 top-1/2 -translate-y-1/2 z-50 rounded-lg border border-white/10 bg-zinc-800 p-3 shadow-xl"
            >
        ...
        <div v-if="tierlist.pool.length > 0" class="mt-6">
          <h2 class="text-sm font-semibold text-zinc-400 mb-3">Pool ({{ tierlist.pool.length }} items)</h2>
