...
export const useRoomStore = defineStore('room', () => {
    ...
    const _pendingAdd = ref(null);
    function handleDragAdded(itemId, toRowId, toIndex) {
        _pendingAdd.value = { itemId, toRowId, toIndex };
    }
    function handleDragRemoved(itemId, fromRowId) {
    ...
    function bindEvents() {
        const { socket } = useSocket();
        if (!socket.value || socket.value === boundSocket)
            return;
        boundSocket = socket.value;
        socket.value.on('room:state', (room) => {
        ...
        socket.value.on('item:moved', (data) => {
            applyRemoteMove(data);
        });
        socket.value.on('room:user-joined', (user) => {
            users.value.push(user);
        });
        socket.value.on('room:user-left', (userId) => {
            users.value = users.value.filter((u) => u.id !== userId);
        });
        socket.value.on('item:created', (item) => {
        ...
        socket.value.on('room:locked', (locked) => {
            isLocked.value = locked;
        });
        socket.value.on('room:focus-toggled', (focused) => {
        ...
        socket.value.on('room:reset', (room) => {
            currentRoom.value = room;
            users.value = room.users;
            title.value = room.tierList.title;
            rows.value = room.tierList.rows;
        ...
        socket.value.on('row:reordered', (data) => {
            ...
            if (newIdx < 0 || newIdx >= rows.value.length)
                return;
    ...
    function updateRow(data) {
        const { socket } = useSocket();
        if (socket.value?.connected)
            socket.value.emit('row:update', data);
        // Optimistic update
        const row = rows.value.find((r) => r.id === data.rowId);
        if (row) {
            if (data.label !== undefined)
    ...
    function deleteRow(data) {
        const { socket } = useSocket();
        if (socket.value?.connected)
            socket.value.emit('row:delete', data);
        const idx = rows.value.findIndex((r) => r.id === data.rowId);
        if (idx !== -1) {
            const removed = rows.value.splice(idx, 1)[0];
            pool.value.push(...removed.items);
    ...
    function reorderRow(data) {
        const { socket } = useSocket();
        if (socket.value?.connected)
            socket.value.emit('row:reorder', data);
        const idx = rows.value.findIndex((r) => r.id === data.rowId);
        if (idx === -1)
            return;
        const newIdx = data.direction === 'up' ? idx - 1 : idx + 1;
    ...
    function applyRemoteMove(data) {
        ...
        if (fromRowId === null) {
            ...
            if (idx !== -1)
                item = pool.value.splice(idx, 1)[0];
        }
        else {
     ...
     */
    function moveItem(itemId, fromContainerId, toContainerId, newIndex) {
    ...
    function loadTierList(tierList) {
